...
const STATUS_COLOR: Record<AttendanceStatus, string> = {
  present: "bg-emerald-600", absent: "bg-rose-600", late: "bg-amber-500", excused: "bg-slate-400"
};
...
export default function AttendancePage() {
  ...
  useEffect(() => {
    Promise.all([listMyClasses(), listSessions(), listTerms()]).then(([c, s, t]) => {
      setClasses(c); setSessions(s); setTerms(t);
  ...
  useEffect(() => {
    ...
    (async () => {
      const [studentList, existing] = await Promise.all([
        listStudentsForAttendance(classId), getAttendanceForDate(classId, date)
      ]);
      setStudents(studentList);
      const initial: Record<string, AttendanceStatus> = {};
      studentList.forEach((s: any) => { initial[s.id] = "present"; });
      existing.forEach((e: any) => { initial[e.student_id] = e.status; });
      setMarks(initial);
      setSaved(false);
    })();
  }, [classId, date]);
  ...
  async function handleSave() {
    ...
    const { error } = await markAttendance({ classId, sessionId: currentSession.id, termId: currentTerm.id, date, records });
    setSaving(false);
  ...
  return (
    <div className="space-y-4 p-4 sm:p-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Mark attendance for {profile?.full_name}'s classes.</p>
      </div>
      ...
      <Card className="overflow-x-auto p-0">
        <table className="w-full min-w-[500px] text-left text-sm">
          <thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500">
            <tr><th className="px-4 py-3">Student</th><th className="px-4 py-3">Status</th></tr>
          </thead>
          <tbody>
            {students.length === 0 && <tr><td colSpan={2} className="px-4 py-6 text-center text-slate-400">No students in this class yet.</td></tr>}
            {students.map((s) => (
              <tr key={s.id} className="border-b border-slate-100 last:border-0">
                <td className="px-4 py-3 font-medium">{s.full_name} <span className="text-xs text-slate-400">{s.admission_number}</span></td>
                  ...
                  <div className="flex gap-1.5">
                    {STATUSES.map((st) => (
                      <button
                        ...
                        className={`rounded-full px-2.5 py-1 text-xs font-medium capitalize text-white ${marks[s.id] === st ? STATUS_COLOR[st] : "bg-slate-200 text-slate-600"}`}
                      >
                        {st}
                      </button>
                    ))}
